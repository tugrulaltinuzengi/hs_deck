...
def _bootstrap_from_library(parse: Callable[[Iterator[Tuple[str, Any]]], None], path=None):
	# LOCAL MODIFICATION: upstream imports `hearthstone_data` unconditionally,
	# so an explicit path still required the data package to be installed.
	# The import now happens only when there is no path to read.
	if path is None:
